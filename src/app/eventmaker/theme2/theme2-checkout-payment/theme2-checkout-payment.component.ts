...
@Component({
  selector: 'theme2-checkout-payment',
  templateUrl: './theme2-checkout-payment.component.html',
  styleUrls: ['./theme2-checkout-payment.component.scss']
})
export class Theme2CheckoutPaymentComponent implements OnInit {
...
ngOnInit(): void {
  ...
  this._eventmakerService.getEventWebsiteDetail(this.eventCheckOut.EventId).subscribe((eventDetails: EventWebsiteModel) => {
    if (eventDetails) {
      this.eventId = this.eventCheckOut.EventId;
      this.themeName = eventDetails.TemplateId;
      eventDetails.EventPages.forEach(res => {
        if (res.PageSystemName === 'event') {
          this.eventCustomPages = res.PageSections.filter(x => x.Selected && x.PageSystemName === "refund-policy")[0];
        }
      });
    }
  });
